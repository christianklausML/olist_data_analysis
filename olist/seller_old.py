...
class Seller:
    ...
    def remove_sellers(n):
        '''
        Returns a DataFrame with:
        'seller
        '''
        tmp = np.arange(n)
        sellers_removed = sellers.sort_values('profits').reset_index().copy()
        sellers_removed.drop(tmp, inplace=True)
        
        return sellers_removed
    ...
    def get_review_score(self):
        ...
        order_items = Order().data['order_items'].copy()
        order_items = order_items[['order_id', 'seller_id']].drop_duplicates().copy()
        
        seller_score = order_items.merge(review_score, on='order_id')
        
        seller_score = seller_score.groupby('seller_id').agg(
            share_of_five_stars = pd.NamedAgg(column='dim_is_five_star', aggfunc='mean'),
            share_of_one_stars = pd.NamedAgg(column='dim_is_one_star', aggfunc='mean'),
            review_score = pd.NamedAgg(column='review_score', aggfunc='mean'),
            count_of_one_star = pd.NamedAgg(column='dim_is_one_star', aggfunc='sum'),
            count_of_two_star = pd.NamedAgg(column='dim_is_two_star', aggfunc='sum'),
            count_of_three_star = pd.NamedAgg(column='dim_is_three_star', aggfunc='sum'),
            count_of_four_star = pd.NamedAgg(column='dim_is_four_star', aggfunc='sum'),
            count_of_five_star = pd.NamedAgg(column='dim_is_five_star', aggfunc='sum'),
            )
